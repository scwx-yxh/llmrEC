...
from LLM_CE_modal import LLM4Rec
...
def train(
    ...
    if task_type == 'general':
        ...
        if data_path == "movielens":
            v_feat = torch.tensor(np.load("/home/share/yangxuanhui/dataset/movielens/FeatureVideo_normal.npy", allow_pickle=True), dtype=torch.float)
            a_feat = torch.tensor(np.load("/home/share/yangxuanhui/dataset/movielens/FeatureAudio_avg_normal.npy", allow_pickle=True), dtype=torch.float)
            t_feat = torch.tensor(np.load("/home/share/yangxuanhui/dataset/movielens/FeatureText_stl_normal.npy", allow_pickle=True), dtype=torch.float)
        elif data_path == "tiktok":
            v_feat = torch.load('/home/share/yangxuanhui/dataset/tiktok/visual_feat_new.pt').to(dtype=torch.float)
            a_feat = torch.load('/home/share/yangxuanhui/dataset/tiktok/audio_feat_new.pt').to(dtype=torch.float)
            t_feat = torch.tensor(np.load('/home/share/yangxuanhui/dataset/tiktok/tiktok_t_64.npy')).to(dtype=torch.float)
        elif data_path == "kwai":
            v_feat = torch.load('/home/share/yangxuanhui/dataset/kwai/v_feat.pt').to(dtype=torch.float)
            a_feat = None
            t_feat = torch.tensor(np.load('/home/share/yangxuanhui/dataset/kwai/kwai_t_64.npy')).to(dtype=torch.float)
            
        data_collator = BipartiteGraphCollator(max_len=max_len)

    prompter = Prompter(prompt_template_name)
    
    train_edge = np.load("/home/share/yangxuanhui/dataset/"+data_path+'/train.npy',allow_pickle=True)
    user_item_dict=np.load("/home/share/yangxuanhui/dataset/"+data_path+'/user_item_dict.npy',allow_pickle=True).item()
    ...
    model = LLM4Rec(
        ...
        modal_feat=[v_feat, a_feat, t_feat],
        mmgcn_parm=[train_edge,user_item_dict]
    )
    ...
    topk, dataloader_bs = 10, 4 if model == "true" else 4
    ...
    for inputs, inputs_mask, labels, neg_item, item_len in test_dataloader:
        _, score_matrix, _, score_matrix_ori, user_embedding, item_embedding = model.predict(inputs.cuda(), inputs_mask.cuda())
        ...
        for row in range(score_matrix_ori.size(0)):
            for col in range(item_len[row]):
                score_matrix_ori[row][items[row][col]] = 1e-5
        ...
        #_, index_of_rank_list_5 = torch.topk(score_matrix, 5)
        #_, index_of_rank_list_5_1 = torch.topk(score_matrix_ori, 5)
        #all_index_of_rank_list_10 = torch.cat((all_index_of_rank_list_10, index_of_rank_list_5.cpu(), index_of_rank_list_5_1.cpu()), dim=0)

        all_index_of_rank_list_10 = all_index_of_rank_list_10.view(-1, 10)

        #_, index_of_rank_list_20 = torch.topk(score_matrix_ori)
        _, index_of_rank_list_10 = torch.topk(score_matrix, 10)
        _, index_of_rank_list_10_1 = torch.topk(score_matrix_ori, 10)
        all_index_of_rank_list_20 = torch.cat((all_index_of_rank_list_20, index_of_rank_list_10.cpu(), index_of_rank_list_10_1.cpu()), dim=0)
        ...
        #np.save('/home/share/yangxuanhui/embedding/item.npy', item_imbedding)
